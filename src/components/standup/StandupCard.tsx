...
import { EditableStandupBody } from '@/components/standup/EditableStandupBody';
...
export function StandupCard({ standup, confirmations = [], showProfile = true, compact = false, canEdit = false, currentUserId, currentUserName, initialComments, views }: Props) {
  ...
  return (
    <div className={`rounded-xl border bg-zinc-900 shadow-sm ${hasBlockers ? 'border-orange-500/30' : 'border-zinc-800'}`}>
      ...
      <div className="p-4 space-y-4">
        ...
        {canEdit ? (
          <EditableStandupBody standup={standup} confirmations={confirmations} />
        ) : (
          <>
            {!compact && (
